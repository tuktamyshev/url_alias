...
from url_alias.infrastructure.persistence.adapters.click import SQLAlchemyClickRepository
...
class SQLAlchemyRepositoriesProvider(Provider):
    ...
    @provide(scope=Scope.REQUEST)
    def click_repository(self, session: AsyncSession) -> ClickRepository:
        return SQLAlchemyClickRepository(session=session)
